...
// 执行修改约束的函数
async function modifyAddressConstraint() {
  const client = await pool.connect();
  
  try {
    console.log('正在检查address列的当前约束...');
    ...
    if (checkResult.rows.length === 0) {
      console.error('找不到customers表中的address列');
      return;
    }
    ...
    if (column.is_nullable === 'YES') {
      console.log('address列已经允许为NULL值，无需修改');
      return;
    }
    
    console.log('正在修改address列约束，设置为允许NULL值...');
    ...
    console.log('成功修改address列约束！现在允许为NULL值');
