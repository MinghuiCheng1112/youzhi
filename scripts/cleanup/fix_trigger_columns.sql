...
DO $$
BEGIN
  RAISE NOTICE '开始修复触发器函数中对不存在列的引用...';
END $$;
...
-- 3. 重新创建capture_soft_deleted_customer函数（移除对不存在列的引用）
...
-- 4. 重新创建record_deleted_customer函数（移除对不存在列的引用）
...
DO $$
BEGIN
  RAISE NOTICE '修复完成！触发器函数中不再引用不存在的列';
END $$;
