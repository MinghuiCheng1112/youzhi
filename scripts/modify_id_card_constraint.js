...
// 执行修改约束的函数
async function modifyIdCardConstraint() {
  const client = await pool.connect();
  
  try {
    console.log('正在检查id_card列的当前约束...');
    
    // 首先检查列的当前约束
    const checkResult = await client.query(`
      SELECT column_name, is_nullable, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'customers' AND column_name = 'id_card'
    `);
    
    if (checkResult.rows.length === 0) {
      console.error('找不到customers表中的id_card列');
      return;
    }
    ...
    if (column.is_nullable === 'YES') {
      console.log('id_card列已经允许为NULL值，无需修改');
      return;
    }
    
    console.log('正在修改id_card列约束，设置为允许NULL值...');
    
    // 执行ALTER TABLE语句修改约束
    await client.query(`
      ALTER TABLE customers ALTER COLUMN id_card DROP NOT NULL;
    `);
    
    console.log('成功修改id_card列约束！现在允许为NULL值');
