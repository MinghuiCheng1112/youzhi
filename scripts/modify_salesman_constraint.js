...
// 执行修改约束的函数
async function modifySalesmanConstraint() {
  const client = await pool.connect();
  
  try {
    console.log('正在检查salesman列的当前约束...');
    
    // 首先检查列的当前约束
    const checkResult = await client.query(`
      SELECT column_name, is_nullable, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'customers' AND column_name = 'salesman'
    `);
    
    if (checkResult.rows.length === 0) {
      console.error('找不到customers表中的salesman列');
      return;
    }
    ...
    if (column.is_nullable === 'YES') {
      console.log('salesman列已经允许为NULL值，无需修改');
      return;
    }
    
    console.log('正在修改salesman列约束，设置为允许NULL值...');
    
    // 执行ALTER TABLE语句修改约束
    await client.query(`
      ALTER TABLE customers ALTER COLUMN salesman DROP NOT NULL;
    `);
    
    console.log('成功修改salesman列约束！现在允许为NULL值');
